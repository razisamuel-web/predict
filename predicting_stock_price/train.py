...
class Train(object):
    ...
    def fittrainGsCv(self, X_train, y_train):
        ...
        grid_search.coef_ = grid_search.best_estimator_._final_estimator.coef_[:] # .coef_[1:] in the past the first coef was all the time 0 in some point its changed
...
class ClfTrain(object):
    ...
    def _pipelinedef(self,params = False):
        if not params:
            ...
            logistic = linear_model.LogisticRegression() if not params['logistic'] else linear_model.LogisticRegression(**params['logistic'])
