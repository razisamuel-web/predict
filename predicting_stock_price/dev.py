...
df_weekly = get_data(main_ticker,
                     ...
                     index_type='object')
# %%
# Retrieve rows from given time period
same_date_last_year = str(datetime.strptime(date_reference, "%Y-%m-%d") - timedelta(days=round(years_back * 365)))
dates = df_weekly.reset_index()['first_day_in_week']
start = dates[dates > same_date_last_year].index[0]
end = dates[dates < date_reference].index[-1]
df_weekly = df_weekly.iloc[start:end, :]

# %%
start, end, same_date_last_year, date_reference
# %%
